...
function TestApp() {
	...
	// 	setProducts((oldState) => {const productsIndex = oldState.findIndex((item) => item.id === productId);
	// 		if (productsIndex !== -1) {
...
function onQuantityChange(productId, count) { // Takes in a productID and count variable.
    setProducts((currentState) => {
        const productsIndex = currentState.findIndex((item) => item.id === productId);
        if (productsIndex !== -1) {
            currentState[productsIndex].count = count; 
        }
        return [...currentState];
    })
}
	...
	return (
		...
		<div className="App">
			<ShoppingCart visibilty={cartsVisibilty} products={productsInCart} onClose={() => setCartVisible(false)} onQuantityChange={onQuantityChange} onProductRemove={onProductRemove} />

			<div className="navbar">
				<h3 className="logo">Logo</h3>
				<button className="btn shopping-cart-btn" onClick={() => setCartVisible(true)}><GiShoppingBag size={24} />
